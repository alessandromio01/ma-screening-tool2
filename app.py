...
if show_technical_details:
    ...
    st.markdown(
        """
        **Pipeline Nearest Neighbors**

        1. Selezione feature numeriche e categoriche.
        2. Missing values: mediana per numeriche, `"Unknown"` per categoriche.
        3. Encoding delle variabili categoriche con `pd.get_dummies`.
        4. Scaling con `StandardScaler`.
        5. Costruzione del profilo medio della bidder.
        6. Ricerca delle target più vicine con `NearestNeighbors(metric="euclidean")`.
        """
    )

    st.code(
        """
categorical_dummies = pd.get_dummies(model_data[categorical_features])

X_raw = pd.concat(
    [model_data[numeric_features], categorical_dummies],
    axis=1
)

X_scaled = StandardScaler().fit_transform(X_raw)

bidder_profile_vector = X_scaled.loc[bidder_indices].mean(axis=0)

knn = NearestNeighbors(n_neighbors=200, metric="euclidean")
distances, indices = knn.kneighbors(bidder_profile_vector)
        """,
        language="python"
    )
    ...
    st.markdown(
        ...
        **Pipeline Random Forest**
        ...
        La Random Forest viene usata come parte supervised del progetto.
        Il target è `price_disclosed`, cioè se il prezzo dell'operazione è dichiarato oppure no.
        """
    )
